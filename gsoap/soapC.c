...
SOAP_SOURCE_STAMP("@(#) soapC.c ver 2.8.71 2018-11-15 04:27:23 GMT")
...
{
	struct methodCall *p;
	struct methodCall *a = (struct methodCall*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct methodCall));
	for (p = a; p && n--; p++)
...
{
	struct fault *p;
	struct fault *a = (struct fault*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct fault));
	for (p = a; p && n--; p++)
...
SOAP_FMAC3 struct methodResponse * SOAP_FMAC4 soap_new_methodResponse(struct soap *soap, int n)
{
	...
	struct methodResponse *a = (struct methodResponse*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct methodResponse));
	for (p = a; p && n--; p++)
...
{
	struct param *p;
	struct param *a = (struct param*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct param));
	for (p = a; p && n--; p++)
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			...
			{	if (a->param == NULL)
				{	if (soap_blist_param == NULL)
					...
					a->param = (struct param *)soap_push_block_max(soap, soap_blist_param, sizeof(struct param));
					if (a->param == NULL)
						return NULL;
...
{
	struct params *p;
	struct params *a = (struct params*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct params));
	for (p = a; p && n--; p++)
...
{
	struct _array *p;
	struct _array *a = (struct _array*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct _array));
	for (p = a; p && n--; p++)
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			...
			{	if (a->value == NULL)
				{	if (soap_blist_value == NULL)
					...
					a->value = (struct value *)soap_push_block_max(soap, soap_blist_value, sizeof(struct value));
					if (a->value == NULL)
						return NULL;
					soap_default_value(soap, a->value);
				}
...
{
	struct data *p;
	struct data *a = (struct data*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct data));
	for (p = a; p && n--; p++)
...
{
	struct member *p;
	struct member *a = (struct member*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct member));
	for (p = a; p && n--; p++)
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			...
			{	if (a->member == NULL)
				{	if (soap_blist_member == NULL)
					...
					a->member = (struct member *)soap_push_block_max(soap, soap_blist_member, sizeof(struct member));
					if (a->member == NULL)
						return NULL;
...
{
	struct _struct *p;
	struct _struct *a = (struct _struct*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct _struct));
	for (p = a; p && n--; p++)
...
{
	struct _base64 *p;
	struct _base64 *a = (struct _base64*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct _base64));
	for (p = a; p && n--; p++)
...
{
	struct value *p;
	struct value *a = (struct value*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct value));
	for (p = a; p && n--; p++)
...
{
	wchar_t * *p;
	wchar_t * *a = (wchar_t **)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(wchar_t *));
	for (p = a; p && n--; p++)
